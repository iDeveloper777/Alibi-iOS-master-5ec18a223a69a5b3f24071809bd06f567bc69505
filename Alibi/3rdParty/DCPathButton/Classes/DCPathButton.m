...
- (void)configureViewsLayoutWithButtonSize:(CGSize)centerButtonSize {
    ...
    self.bloomSize = CGSizeMake(220, 65);
    
    self.bloom = NO;
    self.bloomRadius = 400.0f;
    self.bloomAngel = 25.0f;
    
    // Configure the view's center, it will change after the frame folded or bloomed
    //
    self.foldCenter = CGPointMake(self.bloomSize.width / 2, self.bloomSize.height + 360.0f);
