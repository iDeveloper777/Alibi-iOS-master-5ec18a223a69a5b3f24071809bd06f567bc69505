...
- (IBAction)buttonRegisterTouchUpInside:(id)sender {
    ...
    if (!self.textFieldUsername.text.length) {
        ...
        [hud show:YES];
        [sharedConnect loginUserWithUsername:self.textFieldUsername.text andPassword:self.textFieldPassword.text onCompletion:^(AUser *user, ServerResponse serverResponseCode) {
            [hud hide:YES];
            if (serverResponseCode == OK) {
                [self dismissViewControllerAnimated:YES completion:^{
                    AAppDelegate *appDelegate = (AAppDelegate *)[UIApplication sharedApplication].delegate;
                    ATimelineViewController *timelineViewController = (ATimelineViewController *)[appDelegate.tabBarController.viewControllers[0] topViewController];
