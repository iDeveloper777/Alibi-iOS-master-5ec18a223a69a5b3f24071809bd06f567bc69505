...
- (IBAction)buttonRegisterTouchUpInside:(id)sender {
    ...
    if (!self.textFieldEmail.text.length) {
        ...
        if (self.segmentedControlUserType.selectedSegmentIndex == 1) {
            if (!self.textFieldPhone.text.length) {
                ...
                if (self.mapViewLocation.annotations.count) {
                    ...
                    if (annotations.count) {
                        ...
                        [sharedConnect registerUserWithUsername:self.textFieldUsername.text password:self.textFieldPassword.text email:self.textFieldEmail.text userAvatar:self.imageViewAvatar.image userType:AUserTypeCompany userFullName:self.textFieldFullName.text userInfo:@"" latitude:coordinate.latitude longitude:coordinate.longitude companyAddress:self.textFieldAddress.text companyPhone:self.textFieldPhone.text companyWeb:self.textFieldWeb.text instagramUserId:@"" instagramUsername:@"" onCompletion:^(AUser *user, ServerResponse serverResponseCode) {
                            [hud hide:YES];
            ...
            [sharedConnect registerUserWithUsername:self.textFieldUsername.text password:self.textFieldPassword.text email:self.textFieldEmail.text userAvatar:self.imageViewAvatar.image userType:AUserTypePerson userFullName:self.textFieldFullName.text userInfo:@"" latitude:0 longitude:0 companyAddress:@"" companyPhone:@"" companyWeb:@"" instagramUserId:@"" instagramUsername:@"" onCompletion:^(AUser *user, ServerResponse serverResponseCode) {
                [hud hide:YES];
                if (serverResponseCode == OK) {
                    [self dismissViewControllerAnimated:YES completion:nil];
                } else if (serverResponseCode == NO_CONNECTION) {
...
- (IBAction)handleLongTapGesture:(UILongPressGestureRecognizer *)sender {
    ...
    if (CLLocationCoordinate2DIsValid(coordinate)) {
        ...
        AUser *companyUser = [[AUser alloc] init];
