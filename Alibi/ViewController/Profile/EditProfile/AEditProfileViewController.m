...
@implementation AEditProfileViewController
...
- (void)viewDidLoad {
    ...
    if (sharedConnect.currentUser.userType == AUserTypeCompany) {
    ...
    } else if (sharedConnect.currentUser.userType == AUserTypePerson) {
    ...
    NSURL *avatarURL = [NSURL URLWithString:sharedConnect.currentUser.companyWeb];
...
- (void)barButtonItemSaveTouchUpInside:(UIBarButtonItem*)barButtonItemSave {
    ...
    if (!self.textFieldEmail.text.length) {
        ...
        if (sharedConnect.currentUser.userType == AUserTypeCompany) {
            if (!self.textFieldPhone.text.length) {
                ...
                if (self.mapViewLocation.annotations.count) {
                    ...
                    if (annotations.count) {
                        ...
                        [sharedConnect updateUserWithUserID:sharedConnect.currentUser.userID userType:AUserTypeCompany userEmail:self.textFieldEmail.text password:password userAvatar:image userFullName:self.textFieldFullName.text userInfo:@"" latitude:coordinate.latitude longitude:coordinate.longitude companyAddress:self.textFieldAddress.text companyPhone:self.textFieldPhone.text companyWeb:self.textFieldWeb.text onCompletion:^(AUser *user, ServerResponse serverResponseCode) {
                            [hud hide:YES];
            ...
            [sharedConnect updateUserWithUserID:sharedConnect.currentUser.userID userType:AUserTypePerson userEmail:self.textFieldEmail.text password:password userAvatar:image userFullName:self.textFieldFullName.text userInfo:@"" latitude:0 longitude:0 companyAddress:@"" companyPhone:@"" companyWeb:@"" onCompletion:^(AUser *user, ServerResponse serverResponseCode) {
                [hud hide:YES];
                self.mapViewLocation.delegate = nil;
                [self.mapViewLocation setUserTrackingMode:MKUserTrackingModeNone];
                [self.navigationController popViewControllerAnimated:YES];
...
- (IBAction)buttonSelectAvatarTouchUpInside:(UIButton *)sender {
    ...
    AAppDelegate *appDelegate = (AAppDelegate*)[UIApplication sharedApplication].delegate;
