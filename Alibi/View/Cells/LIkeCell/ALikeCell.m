...
static ALikeCell *sharedCell = nil;

@implementation ALikeCell
...
+ (CGSize)sizeWithLike:(ALike *)like {
    ...
    if (!sharedCell) {
        sharedCell = [[[NSBundle mainBundle] loadNibNamed:@"ALikeCell" owner:nil options:nil] lastObject];
    }
...
- (void)updateFramesAndDataWithDownloads:(BOOL)downloads {
    ...
    if (self.like) {
        if (downloads) {
            ...
            __weak ALikeCell *weakSelf = self;
